...
interface PlayerProps {
  onCollision: (type: 'obstacle' | 'coin') => void;
}

const Player: React.FC<PlayerProps> = ({ onCollision }) => {
  const playerRef = useRef<THREE.Mesh>(null);
  const [subscribeKeys, getKeys] = useKeyboardControls();
  const { currentLane, setLane, jump, setJump, isJumping, setIsJumping } = useGameStore();
  ...
  }, [subscribeKeys, currentLane, setLane, isJumping, setIsJumping, setJump, lanePositions.length]);
  ...
  useFrame((state, delta) => {
    ...
    // TODO: Collision detection with obstacles/coins
  });

  return (
    <mesh ref={playerRef} position={[lanePositions[currentLane], 0, 0]} castShadow>
...
const Obstacle: React.FC<ObstacleProps> = ({ position, id }) => {
  return (
    <mesh position={position} castShadow data-game-object-type="obstacle" data-game-object-id={id}>
      <boxGeometry args={[0.8, 0.8, 0.8]} />
...
const Coin: React.FC<CoinProps> = ({ position, id }) => {
  return (
    <mesh position={position} castShadow data-game-object-type="coin" data-game-object-id={id}>
...
const Ground: React.FC<GroundProps> = ({ playerZ }) => {
  // Render ground segments ahead of the player
  const segments = [];
  const segmentLength = 20; // Length of one ground segment
  const numSegments = 5; // How many segments to render

  // Calculate the starting Z position to keep segments centered around playerZ
  const startZ = Math.floor(playerZ / segmentLength) * segmentLength - (numSegments / 2) * segmentLength;

  for (let i = 0; i < numSegments; i++) {
    const zPos = startZ + i * segmentLength;
    segments.push(
      <mesh receiveShadow position={[0, -0.5, zPos]} key={i}>
...
const Game: React.FC = () => {
  ...
  const gameSpeed = 0.1; // Z-axis speed
  const obstacleSpawnInterval = 1000; // ms
  const collectibleSpawnInterval = 500; // ms
  const obstacleDespawnZ = playerZ + 20; // Despawn objects far behind the player
  ...
  useFrame((state, delta) => {
    ...
    // Move player (camera follows player's Z-position)
    setPlayerZ(playerZ - gameSpeed);

    // Spawn Obstacles
    if (Date.now() - lastObstacleSpawnTime.current > obstacleSpawnInterval) {
      const lane = Math.floor(Math.random() * lanePositions.length);
      const zOffset = Math.random() * 5 + 10; // Spawn 10-15 units ahead
      const newObstacle = {
    ...
    if (Date.now() - lastCollectibleSpawnTime.current > collectibleSpawnInterval) {
      const lane = Math.floor(Math.random() * lanePositions.length);
      const zOffset = Math.random() * 5 + 5; // Spawn 5-10 units ahead
      const newCoin = {
        id: Math.random().toString(),
    ...
    // Despawn old objects (obstacles and collectibles)
    setObstacles(prev => prev.filter(obj => obj.position[2] > playerZ + 2));
    setCollectibles(prev => prev.filter(obj => obj.position[2] > playerZ + 2));

    // Update score based on distance
    setScore(score + delta * 100); // 100 points per second
  });

  useEffect(() => {
    if (gameOver) {
      console.log('Game Over! Final Score:', Math.floor(score));
      // TODO: Navigate to score display / game over screen
      // For now, reset and go back to main menu
      setTimeout(() => {
        // Reset game state
  ...
  useEffect(() => {
    setGameStarted(true);
    return () => {
      useGameStore.getState().resetGame(); // Ensure state is reset on unmount
    };
  }, [setGameStarted]);
  ...
  return (
    <div className="w-full h-full relative">
      ...
      <Canvas shadows camera={{ position: [0, 2, 5], fov: 75 }}>
        <PerspectiveCamera makeDefault position={[0, 2, playerZ + 5]} /> {/* Camera follows player */}
...
const Player: React.FC<PlayerProps> = ({ onCollision }) => {
  const playerRef = useRef<THREE.Mesh>(null);
  const [subscribeKeys, getKeys] = useKeyboardControls();
  const { currentLane, setLane, jump, setJump, isJumping, setIsJumping, playerZ } = useGameStore();
...
const Ground: React.FC<GroundProps> = ({ playerZ }) => {
  ...
  // Calculate the starting Z position to keep segments centered around playerZ
  // We want to render segments from playerZ - X to playerZ + Y
  const minZ = playerZ - segmentLength * 1.5; // Start a bit behind the player
  const maxZ = playerZ - segmentLength * 1.5 + numSegments * segmentLength; // Extend ahead

  for (let i = 0; i < numSegments; i++) {
    // Ensure Z position is always behind or at player and extends forward
    const zPos = Math.floor((playerZ - (numSegments / 2) * segmentLength) / segmentLength) * segmentLength + i * segmentLength;
    segments.push(
      <mesh receiveShadow position={[0, -0.5, zPos]} key={i + zPos} > {/* Key needs to be unique and change with Z */}
        <boxGeometry args={[5, 1, segmentLength]} />
        <meshStandardMaterial color="#4CAF50" />
      </mesh>
    );
...
const Game: React.FC = () => {
  ...
  useEffect(() => {
    if (gameOver) {
      ...
      // TODO: Here, you would typically save the score if logged in
      // navigate to a specific Game Over screen or display modal
      // For now, it resets and goes back to main menu after a delay.
    }
